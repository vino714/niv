...
When /^I select the first item from collection$/ do
 ...
 page.find(:xpath, '//*[@id="thumbnails"]/div[1]/div[1]/div[1]/a/img').click
  ...
  # wait_until{page.find(:xpath, "//*[@id='image_530010_4_wr']").visible?}
  # page.find(:xpath, "//*[@id='image_530010_4_wr']").click
  # sleep 10
 ...
 # end
#  
  # And /^I add item (\d+) to bag$/ do|number|
    # sleep 10
    # product_list = @ids_data_file['product_list']
    # sleep 10
    # wait_until{page.find(:xpath, "//div[@id='#{product_list}']/div[#{number}]/div[4]/img").visible?}
    # page.find(:xpath, "//div[@id='#{product_list}']/div[#{number}]/div[4]/img").click
  # end
#  
  # When /^I add item (\d+) to bag$/ do |number|
 # # Find if there is color and size option exists
 # # if color is there select color 
 # # if size is there select the available size 
 # # Select Color
 # sleep 10
# #wait_until { page.find(:xpath, '//*[@id="pdpAttributes"]').visible? }  
# if (page.has_xpath?('//*[@id="pdpAttributes"]/div[@class="colors"]'))
   # @xpath_color = "//*[@id='pdpAttributes']/div[@class='colors']/ul/li"
   # page.all(:xpath,@xpath_color).sample.click
# end
 # # Select available size
# #wait_until { page.find(:xpath, '//*[@id="pdpAttributes"]').visible? }
# if (page.has_xpath?('//*[@id="pdpAttributes"]/div[@class="sizes"]'))      
  # @xpath_size = "//*[@id='pdpAttributes']/div/div/ul/li"
#  
  # # Iterate through all the sizes and find the available one
  # page.all(:xpath, @xpath_size).each do |node| 
  ...
  # end
# end
# page.find(:xpath, "//*[@alt='add to bag']").click
# end 
    ...
    #i_ctr = 0
    #product_list = @ids_data_file['product_list']
    #wait_until{page.find(:xpath, "//div[#{number}]/div[#{number}]/div[4]/img").visible?}
    #page.find(:xpath, "//div[#{number}]/div[#{number}]/div[4]/img").click
    #sleep 10
 ...
 And /^I retrieve product price for item (\d+)$/ do |number|
   ids_data_file = "config/data/qa/ids.yml"
  @ids_data_file = YAML::load(File.open(ids_data_file))
  product_list = @ids_data_file['product_list']
  ...
  puts "#{@item_dollar_price_as_string}"
end
...
When /^I start checkout$/ do
  wait_until { page.find(:xpath, "//*[@id='#{btnCheckout}']").visible? }
  page.find(:xpath, "//*[@id='#{btnCheckout}']").click
end
...
# Then /^I verify merchandise total$/ do
 ...
 # # page.find(:xpath, "//td[@name='#{merch_total}']").text.should == @item_dollar_price_as_string
  # if (page.find(:xpath, "//td[@name='#{merch_total}']").text == @item_dollar_price_as_string)
   # puts "same"
  # else
  # puts "False"
 # end 
...
#When /^I checkout without profile$/ do
 #     steps %Q{
